...
uint64_t gbToTransfer = 8;

// Function to print help information
void print_help() {
    fprintf(stderr, "Usage: ./MemoryBandwidth_<arch> [options]\n");
    fprintf(stderr, "Options:\n");
    fprintf(stderr, "  -help                Show this help message\n");
    fprintf(stderr, "  -threads <num>      Set the number of threads to use\n");
    fprintf(stderr, "  -shared              Use a shared array for all threads\n");
    fprintf(stderr, "  -private             Use a private array for each thread\n");
    fprintf(stderr, "  -sleep <seconds>     Sleep for a specified number of seconds between tests\n");
    fprintf(stderr, "  -branchinterval <num> Add a branch roughly every <num> bytes\n");
    fprintf(stderr, "  -sizekb <num>       Set the size of the test in KB\n");
    fprintf(stderr, "  -data <num>         Set the base GB to transfer\n");
    fprintf(stderr, "  -autothreads <num>  Test bandwidth scaling up to <num> threads\n");
    fprintf(stderr, "  -method <type>      Set the method for testing (options: scalar, asm, write, copy, cflip, add, instr8, instr4, avx512, sse, sse_write, readbankconflict)\n");
    fprintf(stderr, "\n");
    fprintf(stderr, "Examples:\n");
    fprintf(stderr, "  ./MemoryBandwidth_<arch> -threads 4 -sizekb 1024 -method asm\n");
    fprintf(stderr, "  ./MemoryBandwidth_<arch> -help\n");
}
...
int main(int argc, char *argv[]) {
    ...
    for (int argIdx = 1; argIdx < argc; argIdx++) {
        if (*(argv[argIdx]) == '-') {
            ...
            // Parse -help argument
            if (strncmp(arg, "help", 4) == 0 || strncmp(arg, "help", 2) == 0) {
                print_help();
            ...
            // Parse -sleep argument
                ...
                if (argIdx < argc) {
                    ...
                    fprintf(stderr, "Sleeping for %d seconds between tests\n", sleepTime);
                } else {
                    fprintf(stderr, "Error: -sleep requires a value\n");
                    return 1; // Exit with error
                }
            }
            // Parse -branchinterval argument
                ...
                if (argIdx < argc) {
                    ...
                    fprintf(stderr, "Error: -branchinterval requires a value\n");
                    return 1; // Exit with error
                }
            }
            // Parse -sizekb argument
                ...
                if (argIdx < argc) {
                    ...
                    fprintf(stderr, "Error: -sizekb requires a value\n");
                    return 1; // Exit with error
                }
            }
            // Parse -data argument
            ...
            // Parse -autothreads argument
            else if (strncmp(arg, "autothreads", 11) == 0) {
                argIdx++;
                if (argIdx < argc) {
                    autothreads = atoi(argv[argIdx]);
                    fprintf(stderr, "Testing bandwidth scaling up to %d threads\n", autothreads);
                } else {
                    fprintf(stderr, "Error: -autothreads requires a value\n");
                    return 1; // Exit with error
                }
            }
            // Parse -method argument
                ...
                if (argIdx < argc) {
                    if (strncmp(argv[argIdx], "scalar", 6) == 0) {
                        bw_func = scalar_read; // Assuming scalar_read is defined elsewhere
                        fprintf(stderr, "Using scalar C code\n");
                    } else if (strncmp(argv[argIdx], "asm", 3) == 0) {
                        bw_func = asm_read; // Assuming asm_read is defined elsewhere
                        fprintf(stderr, "Using ASM code (AVX or NEON)\n");
                    } else if (strncmp(argv[argIdx], "write", 5) == 0) {
                        bw_func = asm_write; // Assuming asm_write is defined elsewhere
                        fprintf(stderr, "Using ASM code (AVX or NEON), testing write bandwidth instead of read\n");
                        #ifdef __x86_64
                        if (avx512Supported) {
                            fprintf(stderr, "Using AVX-512 because that's supported\n");
                            bw_func = avx512_write; // Assuming avx512_write is defined elsewhere
                        }
                        #endif
                    } else if (strncmp(argv[argIdx], "copy", 4) == 0) {
                        bw_func = asm_copy; // Assuming asm_copy is defined elsewhere
                        fprintf(stderr, "Using ASM code (AVX or NEON), testing copy bandwidth instead of read\n");
                        #ifdef __x86_64
                        if (avx512Supported) {
                            fprintf(stderr, "Using AVX-512 because that's supported\n");
                            bw_func = avx512_copy; // Assuming avx512_copy is defined elsewhere
                        }
                        #endif
                    } else if (strncmp(argv[argIdx], "cflip", 5) == 0) {
                        bw_func = asm_cflip; // Assuming asm_cflip is defined elsewhere
                        fprintf(stderr, "Using ASM code (AVX or NEON), flipping order of elements within cacheline\n");
                    } else if (strncmp(argv[argIdx], "add", 3) == 0) {
                        bw_func = asm_add; // Assuming asm_add is defined elsewhere
                        fprintf(stderr, "Using ASM code (AVX or NEON), adding constant to array\n");
                        #ifdef __x86_64
                        if (avx512Supported) {
                            fprintf(stderr, "Using AVX-512 because that's supported\n");
                            bw_func = avx512_add; // Assuming avx512_add is defined elsewhere
                        }
                        #endif
                        ...
                        bw_func = avx512_read; // Assuming avx512_read is defined elsewhere
                        fprintf(stderr, "Using ASM code, AVX512\n");
                    } else if (strncmp(argv[argIdx], "sse_write", 9) == 0) {
                        bw_func = sse_write; // Assuming sse_write is defined elsewhere
                        fprintf(stderr, "Using SSE to test write bandwidth\n");
                    } else if (strncmp(argv[argIdx], "sse", 3) == 0) {
                        bw_func = sse_read; // Assuming sse_read is defined elsewhere
                        fprintf(stderr, "Using ASM code, SSE\n");
                    } else if (strncmp(argv[argIdx], "readbankconflict", 17) == 0) {
                        testBankConflict = 1;
                        fprintf(stderr, "Testing for bank conflicts\n");
                    }
                    #endif
                } else {
    ...
    for (int sizeIdx = 0; sizeIdx < testPoints; sizeIdx++) {
        ...
        for (int spacing = 0; spacing <= maxSpacing; spacing++) {
            *arr = spacing;
            ...
            clock_gettime(CLOCK_MONOTONIC, &startTv);
            int rc = readbankconflict(arr, testSize, spacing * 8, totalLoads);
            clock_gettime(CLOCK_MONOTONIC, &endTv);
            time_diff_ns = 1e9 * (endTv.tv_sec - startTv.tv_sec) + (endTv.tv_nsec - startTv.tv_nsec);
            // want loads per ns
            float loadsPerNs = (double)totalLoads / (double)time_diff_ns;
...
float MeasureInstructionBw(uint64_t sizeKb, uint64_t iterations, int nopSize, int branchInterval) {
    ...
    uint64_t time_diff_ns = 1e9 * (endTv.tv_sec - startTv.tv_sec) + ((endTv.tv_nsec - startTv.tv_nsec));
    ...
    bw = 1e9 * gbTransferred / (double)time_diff_ns;
