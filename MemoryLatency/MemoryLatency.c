...
int tlb_test_sizes[33] = {2, 4, 8, 12, 16, 20, 24, 28, 32, 36, 40, 44, 48, 64, 96, 128, 192, 256, 384, 512, 640, 768, 896, 
                               1024, 1152, 1280, 1536, 2048, 3072, 4096, 5120, 6144, 8192};
...
float RunTlbTest(uint32_t entries, uint32_t iterations, uint32_t *preallocatedArr);
...
void print_help() {
    ...
    fprintf(stderr, "  -test <type>         Specify the type of test to run:\n");
    fprintf(stderr, "                        c       - Simple C test\n");
    fprintf(stderr, "                        tlb     - TLB test\n");
    fprintf(stderr, "                        mlp     - Memory parallelism test\n");
    #ifndef UNKNOWN_ARCH
    fprintf(stderr, "                        asm     - ASM (simple address) test\n");
    fprintf(stderr, "                        stlf    - Store to load forwarding test\n");
    fprintf(stderr, "                        matched_stlf - Matched load/store sizes for store to load forwarding test\n");
    #ifndef BITS_32
    fprintf(stderr, "                        dword_stlf - 64-bit store to load forwarding test\n");
    #endif
    #endif
    fprintf(stderr, "  -maxsizemb <value>   Set the maximum test size in MB\n");
    fprintf(stderr, "  -iter <value>        Set the number of iterations for the tests\n");
    #ifndef __MINGW32__
    fprintf(stderr, "  -hugepages           Use huge pages if applicable\n");
    fprintf(stderr, "  -sizekb <value>      Specify a single size in KB for testing\n");
    #endif
    fprintf(stderr, "  -help or --help      Show this help message\n");
}
...
int main(int argc, char* argv[]) {
    ...
    uint32_t tlbTestSizeCount = sizeof(tlb_test_sizes) / sizeof(int);
    int mlpTest = 0;  // if > 0, run MLP test with (value) levels of parallelism max
    int stlf = 0, tlb = 0, hugePages = 0;
        ...
        if (*(argv[argIdx]) == '-') {
            ...
            if (strncmp(arg, "test", 4) == 0) {
                ...
                if (strncmp(testType, "c", 1) == 0) {
                    ...
                    stlf = 2;
                    stlfFunc = stlftest32;
                    fprintf(stderr, "Running store to load forwarding test, with 64-bit stores\n");
                }
                #endif
    ...
    if (mlpTest) {
        ...
        printf("Entries,Latency (ns)\n");
        for (int i = 0; i < tlbTestSizeCount; i++) {
            printf("%d,%f\n", tlb_test_sizes[i], RunTlbTest(tlb_test_sizes[i], ITERATIONS, hugePagesArr));
        }
    } else {
        if (singleSize == 0) {
...
float RunMlpTest(uint32_t size_kb, uint32_t iterations, uint32_t parallelism) {
    ...
    float bw = 1e9 * mbTransferred / (double)time_diff_ns;
...
float RunTlbTest(uint32_t entries, uint32_t iterations, uint32_t *preallocatedArr) {
    ...
    uint32_t size_kb = entries * PAGE_SIZE / 1024;
    uint32_t element_count = size_kb * 1024 / PAGE_SIZE;
    uint32_t list_size = size_kb * 1024 / 4;
    uint32_t sum = 0, current;
    ...
    float cacheLatency = RunTest(4, iterations, preallocatedArr);

    //fprintf(stderr, "Memory used - %u KB, latency: %f, ref latency: %f\n", memoryUsedKb, latency, cacheLatency);
    return fmax(0, latency - cacheLatency);
}
...
void RunStlfTest(uint32_t iterations, int mode) {
    ...
    for (int storeOffset = 0; storeOffset < 64; storeOffset++)
        for (int loadOffset = 0; loadOffset < 64; loadOffset++) {
            ...
            latency = (double) time_diff_ns / (double) iterations;
